...
@Injectable()
export class DementiaSQLiteService {
    constructor(private platform:Platform, public db: SQLite, public storage: Storage) {
      ...
      this.platform.ready().then(() => {
        let db = new SQLite();
          db.openDatabase({
            ...
            db.executeSql('CREATE TABLE IF NOT EXISTS tests (id INTEGER PRIMARY KEY AUTOINCREMENT, name Text, user_id Text, date TIMESTAMP)', {}).then((data) =>{
              console.log("TESTS TABLE CREATED", JSON.stringify(data.res));
            }, (error) => {
              console.log("Error creating tests table", JSON.stringify(error.err));
            });

            db.executeSql('CREATE TABLE IF NOT EXISTS test_sections (id INTEGER PRIMARY KEY AUTOINCREMENT, section INTEGER, question TEXT, score TEXT, question_id INTEGER, test_id INTEGER, CONSTRAINT composite_id UNIQUE (section, question_id, test_id))', {}).then((data) =>{
